...
class vc_secondary: UIViewController {
    ...
    func runTimer() {
        if(isTimerRunning == false){
            timer = Timer.scheduledTimer(timeInterval: 1, target: self,   selector: (#selector(vc_secondary.updateTimer)), userInfo: nil, repeats: true)
            isTimerRunning = true;
        }
